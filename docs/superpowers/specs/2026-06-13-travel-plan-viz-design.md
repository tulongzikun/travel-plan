...
- 机票：给**建议班次**（真实航司+航班号+时刻）；价格只经官方渠道查实时（用户已装的官方 skill，或 `tools/flight_research.py` 直连、key 用户自备；2026-08-21 废除旧「不查实时票价」红线），查到须标查询时间戳与来源、注明以订票页为准，无渠道给参考区间。门票只给参考区间。仍标记"通常需提前预订"的项目（热门景点、跨城高铁 / 机票），赋予推荐提前天数 → 喂给 `reminders.js`。
...
- **"不查实时票价"红线（2026-08-21 修订废除）**：机票实时价可查，但只经官方渠道——用户已装的官方 skill，或 `tools/flight_research.py` 官方 API 直连（key 用户自备）；查到的数据标来源+查询时间戳、注明以订票页为准；不爬 OTA、不代订、不背书。成品页/引擎仍不自建 REST 直连（2026-07-15 拍板不变），直连只发生在调研侧 CLI。
