...
| 🚗 | **Point-to-point transport chain**: every leg (hotel → first stop → between stops → last stop → hotel) with mode + duration (distance estimates for driving, fares for transit), each day's chain ending with the leg to that night's lodging, scheduled against local sunset so no mountain road is driven after dark |
...
- `barcelona-granada-sevilla-cordoba-madrid-11d10n-real.html` — Barcelona, Granada, Seville, Córdoba & Madrid, 11D10N (Mode A: nonstop flight in + AVE rail spine from Shanghai, dates TBD; a Gaudí & Andalusia UNESCO line — Sagrada Família's centenary year, the Alhambra's Court of the Lions, the Mezquita's forest of arches; La Mercè festival-day scheduling, a Monday-closure-proof matrix (Seville Cathedral's short Monday hours moved to Tuesday, Prado's free evening slot), leg-by-leg transit chain and 10 booking reminders; first international sample — mainland-only Amap day links are auto-omitted abroad, 39 dual-sourced WGS-84 points including rail stations and airports, 20 Commons photos all 200-verified)
