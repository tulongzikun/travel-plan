  ...
  hotelAreas: [
    {
      ...
      // 渲染为片区下「🗣 网友推荐」子列表并带"价格为实付当年参考、非实时"脚注；境内行程或没调研到就整个省略（挖掘方法与可信度分层见 research-guide「网友实测情报挖掘」节）。
      crowdPicks: [
  ...
  days: [
    {
      ...
      tips: ["上午先排户外，午后转室内避雷暴"],   // 当日小贴士（可选）；若以「🚗 车程链」汇总当日交通，链条须终于「→当晚住宿」段（与 slot.transport 的逐段链一致，别让住宿腿散在链条外）；链条里每个浏览型点位名后附（浏览约X小时/X分钟）——时长与当日 slot 的 time 字段一致，交通/住宿节点标候车或寄存约 X 分钟（2026-08-23 拍板）
...
1. **页顶**：行程标题 + 出发前待办清单。清单用 `reminders.js` 的 `computeReminders(trip.startDate, trip.reminders)` 再 `renderChecklistHTML(...)` 生成。若 `trip.dateTBD` 为 true，清单上方须加一行醒目提示（如「出发日期未定，以下提醒按 {startDate} 估算，定档后把本页丢回给 AI 重算」）。页顶还须有「⬇ 下载本页 HTML」按钮：点击时克隆 `document.documentElement`、清空地图容器（Leaflet 运行时瓦片不进副本，打开时重新初始化）、补 `<!DOCTYPE html>` 后序列化成 Blob 以页面标题命名下载——线上浏览（GitHub Pages 等）的读者一键拿到离线副本；模板已内置，自设计时照抄该逻辑。
...
- 图片**统一降级**：每个 `<img>` 加 `onerror="this.style.display='none'"`，容器自带与主题配色一致的中性底（纯色/渐变均可，可叠景点名文字）——加载失败或离线时呈现为体面的色块，绝不露浏览器破图图标。`photo` 字段可选：**允许先出无图版**（图片按景点优先级后补，见 research-guide），无图点位直接呈现中性底色块。
