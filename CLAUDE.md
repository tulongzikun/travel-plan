...
- **境外网友推荐酒店（2026-08-23 拍板）**：境外酒店经小红书（`tools/xhs_research.py` 或联网搜）调研网友实测优缺点，写入 `hotelAreas[].crowdPicks`（{name, priceNote 带实付年份, note, source}，渲染带"非实时"脚注）；每片区 1–3 家，素材只作定性参考。
