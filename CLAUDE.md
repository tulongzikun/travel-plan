...
- **图片只补 T0、不阻塞正文（2026-08-21 拍板 + 2026-08-30 收紧）**：正文数据齐就先生成交付（可无图版，`photo` 留空走降级）；补图只做 T0=门面/顶流档（每点 1 张、200 校验后进 trip-data 重渲染），**其余档位留空不查**。Commons：存在性用批量 imageinfo 一次摸底，逐 URL 校验 ≥8s 间隔 + retry 退避（1s 连发第 5 个起 429）；同景区通用图须在 note 如实标注。细则见 research-guide「图片按优先级后补」节。
